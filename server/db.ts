...
export class PaymentError extends Error {
  constructor(public code: "BAD_REQUEST" | "NOT_FOUND" | "PRECONDITION_FAILED", message: string) {
    super(message);
...
export async function registerJobPaymentAtomic(params: {
  ...
  // `injectedDb` es sólo para tests (mismo patrón DI que el canónico). En
  // producción SIEMPRE getDb() real; en Vitest getDb() es fail-closed.
  ...
  return db.transaction(async (tx: any) => {
    ...
    //    Si no está inequívocamente abierto ('false') ⇒ PaymentError MAINTENANCE y
    //    ROLLBACK, ANTES de tocar el job/ledger. Semántica IDÉNTICA al canónico.
